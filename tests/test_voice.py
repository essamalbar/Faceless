...
def test_generate_narration_per_beat_prefers_character_name_voices(tmp_path, monkeypatch):
    """When character_name_voices is supplied, it wins over the speaker-keyed
    legacy character_voices map."""
    from pipeline.voice import generate_narration_per_beat
    from pipeline.types import Beat

    captured = []
    class _StubClient:
        def synthesize(self, *, text, voice_id, model, out_path):
            captured.append({"text": text, "voice_id": voice_id})
            out_path.write_bytes(b"fake-mp3")

    monkeypatch.setattr("pipeline.voice._build_elevenlabs", lambda: _StubClient())
    monkeypatch.setattr("pipeline.voice._audio_duration_ms", lambda p: 1000)
    monkeypatch.setattr("pipeline.voice._audio_duration_ms_safe", lambda p: 1000)
    monkeypatch.setattr("pipeline.voice._ffmpeg_concat_mp3s",
                        lambda parts, out: out.write_bytes(b"combined"))

    beats = [
        Beat(arabic="مرحبا", english_motion="x", clip_duration_s=8.0,
             speaker="son", character_name="خالد"),
        Beat(arabic="مرحبا يا أمي", english_motion="x", clip_duration_s=8.0,
             speaker="mother", character_name="فاطمة"),
    ]
    generate_narration_per_beat(
        beats,
        character_voices={"son": "speaker_son_voice", "mother": "speaker_mother_voice"},
        parts_dir=tmp_path / "parts",
        combined_mp3_path=tmp_path / "combined.mp3",
        timings_path=tmp_path / "timings.json",
        character_name_voices={"خالد": "khaled_voice", "فاطمة": "fatima_voice"},
        fallback_voice_id="default_voice",
    )
    voices_used = [c["voice_id"] for c in captured]
    assert voices_used == ["khaled_voice", "fatima_voice"]


def test_generate_narration_per_beat_falls_back_to_speaker_keyed_map(tmp_path, monkeypatch):
    """If a character_name isn't in character_name_voices, fall back to the
    speaker-keyed map. If that also misses, fall back to fallback_voice_id."""
    from pipeline.voice import generate_narration_per_beat
    from pipeline.types import Beat
    ...
    class _StubClient:
        def synthesize(self, *, text, voice_id, model, out_path):
            captured.append({"voice_id": voice_id})
            out_path.write_bytes(b"fake-mp3")
    ...
    beats = [
        Beat(arabic="ا", english_motion="x", clip_duration_s=8.0,
             speaker="mother", character_name="غير_مسجل"),  # not in name map
        Beat(arabic="ب", english_motion="x", clip_duration_s=8.0,
             speaker="warrior", character_name="مجهول"),  # neither map has it
    ]
    generate_narration_per_beat(
        beats,
        character_voices={"mother": "speaker_mother_voice"},  # speaker-keyed
        parts_dir=tmp_path / "parts",
        combined_mp3_path=tmp_path / "combined.mp3",
        timings_path=tmp_path / "timings.json",
        character_name_voices={},  # empty
        fallback_voice_id="default_narrator",
    )
    voices_used = [c["voice_id"] for c in captured]
    # First beat: name miss → speaker hit ("mother" → speaker_mother_voice)
    # Second beat: name miss + speaker miss → fallback
    assert voices_used == ["speaker_mother_voice", "default_narrator"]
